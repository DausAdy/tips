...
description: Tokenized Vaults with a single underlying TIP-20 token.
author: yanghang8612@gmail.com
discussions-to: https://github.com/tronprotocol/tips/issues/840
status: Last Call
...
The following standard allows for the implementation of a standard API for tokenized Vaults representing shares of a single underlying [TIP-20](https://github.com/tronprotocol/tips/blob/master/tip-20.md) token.

This standard is an extension on the TIP-20 token that provides basic functionality for depositing and withdrawing tokens and reading balances.
...
All [TIP-4626](./840) tokenized Vaults MUST implement TIP-20 to represent shares.
If a Vault is to be non-transferrable, it MAY revert on calls to `transfer` or `transferFrom`.
The TIP-20 operations `balanceOf`, `transfer`, `totalSupply`, etc. operate on the Vault "shares"
which represent a claim to ownership on a fraction of the Vault's underlying holdings.

All TIP-4626 tokenized Vaults MUST implement TIP-20's optional metadata extensions.
The `name` and `symbol` functions SHOULD reflect the underlying token's `name` and `symbol` in some way.

TIP-4626 tokenized Vaults MAY implement [TIP-2612](./tip-2612.md) to improve the UX of approving shares on various integrations.

### Definitions:

- asset: The underlying token managed by the Vault.
  Has units defined by the corresponding TIP-20 contract.
...
MUST be an TIP-20 token contract.
...
MUST support TIP-20 `approve` / `transferFrom` on `asset` as a deposit flow.
...
MUST support TIP-20 `approve` / `transferFrom` on `asset` as a mint flow.
...
MUST support a withdraw flow where the shares are burned from `owner` directly where `msg.sender` has TIP-20 approval over the shares of `owner`.
...
MUST support a redeem flow where the shares are burned from `owner` directly where `msg.sender` has TIP-20 approval over the shares of `owner`.
...
MUST be emitted when shares are withdrawn from the Vault in `TIP-4626.redeem` or `TIP-4626.withdraw` methods.
...
TIP-20 is enforced because implementation details like token approval
and balance calculation directly carry over to the shares accounting.
This standardization makes the Vaults immediately compatible with all TIP-20 use cases in addition to TIP-4626.
...
TIP-4626 is fully backward compatible with the TIP-20 standard and has no known compatibility issues with other standards.
For production implementations of Vaults which do not use TIP-4626, wrapper adapters can be developed and used.
